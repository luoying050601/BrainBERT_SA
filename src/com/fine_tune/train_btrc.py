...
from src.com.pre_train.dataset import (BtmRankDataset, btrc_train_collate,
                                       BtmValDataset, btrc_val_collate,
                                       BtmEvalDataset, btrc_eval_collate)
...
def main(opts):
    ...
    val_dataloader = build_dataloader(val_dataset, btrc_val_collate,
                                      False, opts)
    # eval
    ...
    eval_dataset_val = BtmEvalDataset(val_txt_db, val_brain_db,
                                      opts.inf_minibatch_size)
    eval_loader_val = build_dataloader(eval_dataset_val, btrc_eval_collate,
                                       False, opts)
    ...
    eval_dataset_test = BtmEvalDataset(test_txt_db, test_brain_db,
                                       opts.inf_minibatch_size)
    eval_loader_test = build_dataloader(eval_dataset_test, btrc_eval_collate,
                                        False, opts)
    ...
    while True:
        train_dataloader = build_dataloader(
            train_dataset, btrc_train_collate, True, opts)
        for step, batch in enumerate(train_dataloader):
            n_examples += batch['input_ids'].size(0)
            loss = model(batch=batch, task='btrc', compute_loss=True)
            loss = loss.mean()
...
def validate(model, val_loader):
    ...
    for batch in val_loader:
        scores = model(batch=batch, task='btrc', compute_loss=False)
...
if __name__ == "__main__":
    ...
    args.checkpoint = Proj_dir + "/output/ckpt/model_step_2000.pt"
    args.train_txt_dbs = [
    ...
    args.val_txt_db = Proj_dir + f"/txt/alice/pretrain_val.db"

    args.val_brain_db = Proj_dir + f"/brain/alice/pretrain_val.db"
    args.test_txt_db = Proj_dir + f"/txt/alice/pretrain_test.db"
    # Proj_dir + "/txt/pereira/pretrain_test.db"]
    args.test_brain_db = Proj_dir + f"/brain/alice/pretrain_test.db"
    # Proj_dir + "/brain/pereira/pretrain_test.db"]
    args.negative_size = 1
    args.inf_minibatch_size = 40
    args.margin = 0.2
    args.full_val = True
